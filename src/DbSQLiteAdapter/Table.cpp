...
std::string Table::getName() const { return m_name; }
...
Table::createFieldValue(const IField &field) const {
  return std::unique_ptr<IFieldValue>(new FieldValue(field));
}

std::unique_ptr<IFieldValue> Table::createFieldValue(const IField &field,
                                                     bool value) const {
...
std::unique_ptr<IFieldValue>
Table::createFieldValue(const IField &field, const std::string &value) const {
  return std::unique_ptr<IFieldValue>(new FieldValue(field, value));
}

std::unique_ptr<IFieldValue>
Table::createFieldValue(const IField &field,
...
Table::getRecordByPrimaryKey(const IPrimaryKeyValue &primaryKeyValue) const {
...
Table::filterRecordsByField(const IFieldValue &conditionValue,
                            const IField *orderByField) const {
  std::vector<IFieldValue *> conditionValues;
...
std::unique_ptr<ITableRecordSet>
Table::filterRecordsByFields(const std::vector<IFieldValue *> &conditionValues,
                             const IField *orderByField) const {
  ...
  for (unsigned int j = 0; j < nConditionFieldValues; j++) {
    ...
    if (!isOwned(field)) {
      throw std::string(
          "Can't filter by fields that don't come from this table.");
    }

    if (!conditionFieldValue.isDefault()) {
      std::string conditionFieldValueName = field.getName();
      std::string conditionFieldValueSQLValue =
          getSQLValue(conditionFieldValue, true, false);
      conditionValuesSQL.push_back(conditionFieldValueName +
                                   conditionFieldValueSQLValue);
    }
  }
...
std::unique_ptr<ITableRecordSet>
Table::filterRecordsByCondition(const std::string &SQLCondition) const {
...
std::unique_ptr<ITableRecord> Table::createRecord() const {
  ...
  return std::unique_ptr<ITableRecord>(
      new TableRecord((ITable &)*this, fieldValues));
}

std::unique_ptr<ITableRecord>
Table::copyRecord(const ITableRecord &record) const {
  ...
  return std::unique_ptr<ITableRecord>(
      new TableRecord((ITable &)*this, copyFieldValues));
}
...
RowsAffected Table::insertRecord(ITableRecord &record) {
  ...
  std::string insert = "INSERT INTO " + m_name + " (" + fieldNamesSQLStr +
...
RowsAffected Table::updateRecord(const std::vector<IFieldValue *> &newValues,
                                 const IPrimaryKeyValue &primaryKeyValue) {
  if (&primaryKeyValue.getTable() != this) {
    throw std::string(
        "Can't update records using a primary key value from another table.");
  }
...
RowsAffected Table::deleteRecord(const IPrimaryKeyValue &primaryKeyValue) {
  if (&primaryKeyValue.getTable() != this) {
    throw std::string(
        "Can't delete records using a primary key value from another table.");
  }
...
RowsAffected Table::updateRecordsByCondition(
  ...
  for (unsigned int i = 0; i < nNewFieldValues; i++) {
    ...
    if (!isOwned(field)) {
      throw std::string("Can't update records using new values that aren't "
                        "owned by this table.");
    }

    if (!newFieldValue.isDefault()) {
      std::string newFieldValueName = field.getName();
      std::string newFieldValueSQLValue =
          getSQLValue(newFieldValue, false, true);
  ...
  for (unsigned int j = 0; j < nConditionFieldValues; j++) {
    ...
    if (!isOwned(field)) {
      throw std::string("Can't update records using condition values that "
                        "aren't owned by this table.");
    }
  ...
  if (!newValuesSQL.empty() && !conditionValuesSQL.empty()) {
    ...
    std::string update = "UPDATE " + m_name + " " + "SET " + newValuesSQLStr +
...
RowsAffected Table::deleteRecordsByCondition(
    const std::vector<IFieldValue *> &conditionValues) {
  std::vector<std::string> conditionValuesSQL;
  unsigned int nConditionFieldValues = (unsigned int)conditionValues.size();
  ...
  if (!conditionValuesSQL.empty()) {
    ...
    std::string deleteSQL =
        "DELETE FROM " + m_name + " " + "WHERE " + conditionSQLStr + ";";
...
RowsAffected Table::deleteRecordsByCondition(const std::string &condition) {
  std::string deleteSQL =
      "DELETE FROM " + m_name + " " + "WHERE " + condition + ";";
  m_database.executeOperation(deleteSQL);
  return m_database.getRowsAffectedByLastChangeOperation();
}
...
void Table::loadFields() {
  ...
  std::unique_ptr<IRecordSet> fieldsRecordSet =
      m_database.executeQuery(query, true);
  ...
  while (fieldsRecordSet->isCurrentRecordValid()) {
    ...
    std::unique_ptr<IField> fieldCopy(
        new Field(i, fieldName, fieldType, defaultValue, fieldPK));
...
std::string Table::getSQLValue(const IFieldValue &fieldValue,
                               bool forComparison, bool forAssignment) const {
  ...
  if (fieldValue.isNull()) {
    ...
    case STRING: {
      char *value =
          sqlite3_mprintf("'%q'", fieldValue.getStringValue().c_str());
...
std::string Table::getStringList(const std::vector<std::string> &items,
