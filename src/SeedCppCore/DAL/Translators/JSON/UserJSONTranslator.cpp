...
UserJSONSaveTranslator::~UserJSONSaveTranslator() {}

void UserJSONSaveTranslator::saveEntityToJSON(
    systelab::json_adapter::IJSONValue &jsonUser) const {
  jsonUser.setType(systelab::json_adapter::OBJECT_TYPE);
...
std::string
UserJSONSaveTranslator::translateRoleToString(model::User::Role role) const {
...
UserJSONLoadTranslator::~UserJSONLoadTranslator() {}

void UserJSONLoadTranslator::loadEntityFromJSON(
    const systelab::json_adapter::IJSONValue &jsonUser) {
  ...
  if (jsonUser.hasObjectMember("password")) {
    std::string password =
        jsonUser.getObjectMemberValue("password").getString();
...
model::User::Role
UserJSONLoadTranslator::translateStringToRole(const std::string &role) const {
  if (role == "admin") {
