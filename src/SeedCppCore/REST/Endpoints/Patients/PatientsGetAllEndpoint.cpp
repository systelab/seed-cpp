...
PatientsGetAllEndpoint::PatientsGetAllEndpoint(
    ...
    : m_headers(headers), m_queryStrings(queryStrings),
...
std::unique_ptr<systelab::web_server::Reply> PatientsGetAllEndpoint::execute() {
  ...
  std::unique_ptr<systelab::json_adapter::IJSONValue> jsonContent =
      jsonRoot.buildValue(systelab::json_adapter::ARRAY_TYPE);
  ...
  for (size_t i = 0; i < nPatients; i++) {
    ...
    auto patientTranslator =
        m_jsonTranslatorsFactory.buildPatientSaveTranslator(patient);
  ...
  jsonRoot.addMember("numberOfElements",
                     (int)paginationData->m_numberOfElements);
...
std::unique_ptr<PatientsGetAllEndpoint::PaginationData>
PatientsGetAllEndpoint::getPaginationData() const {
  unsigned int page = 0;
  ...
  unsigned int indexEnd = ((indexBegin + size) < totalElements)
                              ? (indexBegin + size)
                              : totalElements;
  ...
  paginationData->m_totalPages =
      (unsigned int)ceil((double)totalElements / size);
