...
std::unique_ptr<IEndpoint> EndpointsFactory::buildPatientsGetAllEndpoint(
  ...
  service::IAuthorizationValidatorService &authorizationValidatorService =
      m_core.getServicesMgr().getAuthorizationValidatorService();
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildPatientsGetEndpoint(
  ...
  std::string patientId =
      requestData.getRouteParameters()[0].getValue<std::string>();
  model::EntityMgr<model::Patient> &patientMgr =
      m_core.getModel().getPatientMgr();
  dal::IJSONTranslatorsFactory &jsonTranslatorsFactory =
      m_core.getJSONTranslatorsFactory();
  ...
  return std::make_unique<PatientsGetEndpoint>(
      headers, patientId, patientMgr, jsonTranslatorsFactory, jsonAdapter,
      authorizationValidatorService);
}
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildPatientsPostEndpoint(
  ...
  service::IJSONValidatorService &jsonValidatorService =
      m_core.getServicesMgr().getJSONValidatorService();
  service::IPatientModelService &patientModelService =
      m_core.getServicesMgr().getPatientModelService();
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildPatientsDeleteEndpoint(
  ...
  service::IPatientModelService &patientModelService =
      servicesMgr.getPatientModelService();
  service::IAuthorizationValidatorService &authorizationValidatorService =
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildUsersGetEndpoint(
  ...
  return std::make_unique<UsersGetEndpoint>(headers, userId, userMgr,
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildUsersPostEndpoint(
  ...
  service::IUserModelService &userModelService =
      m_core.getServicesMgr().getUserModelService();
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildUsersDeleteEndpoint(
  ...
  service::ServicesMgr &servicesMgr = m_core.getServicesMgr();
  service::IUserModelService &userModelService =
      servicesMgr.getUserModelService();
  service::IAuthorizationValidatorService &authorizationValidatorService =
      servicesMgr.getAuthorizationValidatorService();
...
std::unique_ptr<IEndpoint> EndpointsFactory::buildUsersLoginPostEndpoint(
  ...
  service::IUserModelService &userModelService =
      m_core.getServicesMgr().getUserModelService();
  service::IJWTBuilderService &jwtBuilderService =
      m_core.getServicesMgr().getJWTBuilderService();
